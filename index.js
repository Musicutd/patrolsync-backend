  ...
  try{
    ...
    const data=await withTenant(tenantId,async client=>{
      const [contracts,forecasts,rules,recommendations,decisions]=await Promise.all([
        ...
        client.query(`SELECT COUNT(*)::int total,COUNT(DISTINCT site_id)::int sites,COUNT(*) FILTER(WHERE risk_score<0 OR risk_score>100 OR breach_probability<0 OR breach_probability>100 OR risk_band NOT IN('low','medium','high','critical'))::int invalid,COUNT(*) FILTER(WHERE jsonb_typeof(signals)<>'object' OR jsonb_typeof(reasons)<>'array' OR jsonb_array_length(reasons)=0 OR jsonb_typeof(recommended_actions)<>'array')::int unexplained,COUNT(*) FILTER(WHERE sc.id IS NULL OR s.id IS NULL)::int broken,MAX(calculated_at) latest FROM assurance_risk_forecasts f LEFT JOIN sites s ON s.id=f.site_id AND s.tenant_id=f.tenant_id LEFT JOIN service_contracts sc ON sc.id=f.contract_id AND sc.tenant_id=f.tenant_id WHERE f.tenant_id=$1`,[tenantId]),
        client.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE patrol_miss_pct<0 OR patrol_miss_pct>100 OR incident_miss_pct<0 OR incident_miss_pct>100 OR coverage_miss_pct<0 OR coverage_miss_pct>100 OR maximum_credit_pct<0 OR maximum_credit_pct>100)::int invalid,COUNT(*) FILTER(WHERE sc.id IS NULL)::int broken FROM service_credit_rules r LEFT JOIN service_contracts sc ON sc.id=r.contract_id AND sc.tenant_id=r.tenant_id WHERE r.tenant_id=$1 AND r.active=TRUE`,[tenantId]),
        client.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE status='draft')::int drafts,COUNT(*) FILTER(WHERE status='approved')::int approved,COUNT(*) FILTER(WHERE status='rejected')::int rejected,COUNT(*) FILTER(WHERE credit_percent<0 OR credit_percent>100 OR credit_basis<0 OR recommended_amount<0 OR ABS(recommended_amount-(credit_basis*credit_percent/100))>0.02 OR jsonb_typeof(metrics)<>'object' OR jsonb_typeof(reasons)<>'array')::int invalid,COUNT(*) FILTER(WHERE sc.id IS NULL OR s.id IS NULL)::int broken,COUNT(*) FILTER(WHERE status IN('approved','rejected') AND(decided_by_user_id IS NULL OR decided_at IS NULL OR COALESCE(BTRIM(decision_reason),'')=''))::int missing_decision_evidence FROM service_credit_recommendations r LEFT JOIN service_contracts sc ON sc.id=r.contract_id AND sc.tenant_id=r.tenant_id LEFT JOIN sites s ON s.id=r.site_id AND s.tenant_id=r.tenant_id WHERE r.tenant_id=$1`,[tenantId]),
