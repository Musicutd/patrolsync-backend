...
async function requirePlatformAuth(req,res,next){const header=req.headers.authorization;if(!header?.startsWith('Bearer '))return res.status(401).json({error:'Platform authentication required'});try{const payload=jwt.verify(header.slice(7),PLATFORM_JWT_SECRET,{audience:'patrolsync-platform',issuer:'patrolsync'});if(payload.role!=='platform_admin')throw new Error('Invalid platform role');const found=await pool.query(`SELECT id,email,display_name,active FROM platform_admins WHERE id=$1`,[payload.platform_admin_id]);if(!found.rowCount||!found.rows[0].active)return res.status(401).json({error:'Platform account disabled'});req.platformAdmin=found.rows[0];next()}catch(e){res.status(401).json({error:'Invalid or expired platform session'})}}
...
app.post('/api/platform/auth/verify',fixedWindowRateLimit('platform-mfa',15),async(req,res)=>{const client=await pool.connect();try{await client.query('BEGIN');const found=await client.query(`SELECT c.*,a.email,a.display_name,a.active FROM platform_mfa_challenges c JOIN platform_admins a ON a.id=c.platform_admin_id WHERE c.token_hash=$1 AND c.used_at IS NULL AND c.expires_at>NOW() FOR UPDATE OF c`,[platformDigest(req.body.challenge_token||'')]);if(!found.rowCount){await client.query('ROLLBACK');return res.status(400).json({error:'Verification code is invalid or expired'})}const c=found.rows[0];if(!c.active){await client.query('ROLLBACK');return res.status(401).json({error:'Platform account disabled'})}if(c.attempts>=5){await client.query('ROLLBACK');return res.status(429).json({error:'Too many incorrect attempts'})}const supplied=platformDigest(String(req.body.code||'').trim());if(!crypto.timingSafeEqual(Buffer.from(supplied,'hex'),Buffer.from(c.code_hash,'hex'))){await client.query(`UPDATE platform_mfa_challenges SET attempts=attempts+1 WHERE id=$1`,[c.id]);await client.query('COMMIT');return res.status(400).json({error:'Incorrect verification code'})}await client.query(`UPDATE platform_mfa_challenges SET used_at=NOW() WHERE id=$1`,[c.id]);await client.query(`UPDATE platform_admins SET last_login_at=NOW() WHERE id=$1`,[c.platform_admin_id]);await client.query(`INSERT INTO platform_audit_logs(platform_admin_id,admin_email,action,resource,details,ip_address,request_id) VALUES($1,$2,'LOGIN','platform_console',$3::jsonb,$4,$5)`,[c.platform_admin_id,c.email,JSON.stringify({user_agent:c.user_agent}),requestIp(req),req.requestId]);await client.query('COMMIT');const token=jwt.sign({platform_admin_id:c.platform_admin_id,email:c.email,role:'platform_admin'},PLATFORM_JWT_SECRET,{expiresIn:'4h',audience:'patrolsync-platform',issuer:'patrolsync'});res.json({token,admin:{id:c.platform_admin_id,email:c.email,display_name:c.display_name},expires_in_hours:4})}catch(e){try{await client.query('ROLLBACK')}catch(_){}res.status(500).json({error:'Could not verify platform login'})}finally{client.release()}});
