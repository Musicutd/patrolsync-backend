...
app.get('/api/platform/public-launch-pilots',requirePlatformAuth,async(req,res)=>{try{const rows=(await pool.query(`SELECT DISTINCT ON(se.tenant_id) se.tenant_id,t.name tenant_name,se.details pilot,se.created_at plan_recorded_at FROM system_events se JOIN tenants t ON t.id=se.tenant_id WHERE se.event_type='stage_12_3_pilot_acceptance' ORDER BY se.tenant_id,se.created_at DESC`)).rows;res.json(rows)}catch(e){res.status(500).json({error:e.message,request_id:req.requestId||null})}});

async function buildPlatformPublicLaunchGate(tenantId){
  const started=Date.now(),checks=[],add=(key,label,passed,message,critical=true,details={})=>checks.push({key,label,passed:Boolean(passed),critical,status:passed?'pass':critical?'fail':'warning',message,details});
  const [tenantResult,pilotResult,reviewsResult,exitResult,browserResult,decisionResult,rlsResult,jobsResult,queuesResult,attestationResult,auditResult]=await Promise.all([
  ...
  let startDate=null,endDate=null,duration=0,windowComplete=false,covered=0;if(pilot){startDate=String(pilot.planned_start);duration=Math.max(1,Number(pilot.duration_days||14));endDate=new Date(Date.parse(startDate+'T00:00:00Z')+(duration-1)*86400000).toISOString().slice(0,10);windowComplete=new Date().toISOString().slice(0,10)>endDate;covered=new Set(reviews.map(x=>String(x.review_date).slice(0,10)).filter(d=>d>=startDate&&d<=endDate)).size}
