...
The frontend snapshot contains the production backend URL in 120 HTML/JavaScript files, including `login.html`, `dashboard.html`, `patrolsync-module.js`, and `vision_overview.html`. Render's `patrolsync-frontend` static site deploys `main` automatically and has PR previews disabled. A newly created staging static site from the current frontend branch would still send many requests to the production backend. **Do not create or deploy that staging frontend yet.** First introduce a reviewed, same-origin staging API configuration across every route used by the acceptance flow, with a fail-closed rule that prevents a non-production origin from falling back to the production backend. Validate the candidate build by scanning its served assets for the production API URL and blocking that URL in browser network tests. Do not alter the live `patrolsync.co` site while doing this work.
