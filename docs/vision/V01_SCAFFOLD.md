...
A disposable PostgreSQL CI test exercises the rollout-flag query with the restricted database role and a tenant RLS policy. A second disposable PostgreSQL/Express test runs the actual tenant-scoped flag and entitlement queries through the status route. It confirms that a flag alone is insufficient, a tenant-specific entitlement enables only that tenant, and expiration disables access. That test also loads the unchanged production login handler, tracked-session creation, JWT/session middleware and administrator-permission middleware into an isolated harness. It verifies password rejection, login-issued token access, invalid tokens, revoked sessions, disabled accounts, guards and delegated staff. Another isolated HTTP test covers disabled, denied, role-restricted and schema-failure cases. The complete production server, MFA login path and deployed frontend/backend remain untested against a representative staging environment.
