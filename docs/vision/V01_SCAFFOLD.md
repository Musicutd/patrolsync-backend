...
A disposable PostgreSQL CI test exercises the rollout-flag query with the restricted database role and a tenant RLS policy. It demonstrates that a different tenant context cannot read an enabled flag. This is not yet a full HTTP-route or entitlement integration test.
