...
app.get('/api/launch/production-pilot-readiness',requireAuth,requireOwnerAdmin,async(req,res)=>{
  ...
  try{
    ...
    const [globalData,tenantData]=await Promise.all([
      Promise.all([
        pool.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE c.relrowsecurity)::int enabled,COUNT(*) FILTER(WHERE EXISTS(SELECT 1 FROM pg_policies p WHERE p.schemaname=n.nspname AND p.tablename=c.relname))::int protected,COALESCE(ARRAY_AGG(c.relname ORDER BY c.relname) FILTER(WHERE NOT c.relrowsecurity),'{}'::name[]) missing_rls,COALESCE(ARRAY_AGG(c.relname ORDER BY c.relname) FILTER(WHERE NOT EXISTS(SELECT 1 FROM pg_policies p WHERE p.schemaname=n.nspname AND p.tablename=c.relname)),'{}'::name[]) missing_policy FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace JOIN pg_attribute a ON a.attrelid=c.oid AND a.attname='tenant_id' AND a.attnum>0 AND NOT a.attisdropped WHERE n.nspname='public' AND c.relkind IN('r','p')`),
