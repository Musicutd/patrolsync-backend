...
app.post('/api/incidents', requireAuth, async (req, res) => {
  ...
  const user_id = req.auth.user_id;
  if (!tenant_id || !site_id || !description) return res.status(400).json({ error: 'tenant_id, site_id, and description are required' });
...
app.get('/api/incidents/:id/photos', requireAuth, async (req, res) => {
  ...
  try {
    const result = await withTenant(tenant_id, (client) =>
      client.query('SELECT id, photo_data, created_at FROM incident_photos WHERE incident_id = $1 AND tenant_id = $2 ORDER BY created_at ASC', [id, tenant_id])
    );
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.delete('/api/incidents/:id/photos', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const result = await withTenant(tenant_id, (client) =>
      client.query('DELETE FROM incident_photos WHERE incident_id = $1 AND tenant_id = $2 RETURNING id', [id, tenant_id])
    );
    res.json({ deleted_count: result.rows.length });
  } catch (err) {
    res.status(500).json({ error: err.message });
